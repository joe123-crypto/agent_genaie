...
export async function getSignedInAccountStatus(uid: string) {
  ...
  const hasActiveLink = phoneDoc.exists && isActivePhoneLink(link);
  const phone = hasActiveLink && link.phone ? normalizePhone(link.phone) : null;
  const phoneHash = hasActiveLink ? link.phoneHash || (phone ? whatsappPhoneHash(phone) : null) : null;
  return {
    userId: safeUid,
