...
// own r2-storage functions, so the real client wiring and the <uid>/cv/cv.pdf key
...
test("CV object round-trips: put -> exists -> presigned GET -> delete", opts, async () => {
  const key = cvObjectKey(`ci-uid-${Date.now()}`);
  assert.match(key, /\/cv\/cv\.pdf$/); // layout guard
